...
Subsequent near-term research would involve further synthesis (and proper storage) of the compound, followed by TGA analysis with electromagnet to replicate spin-crossover behavior.

The biggest challenge we ran into in the experimental section of this research is that the compound appears to be air-sensitive, and degrade on prolonged exposure to atmospheric conditions.  Further studies would require us to be more careful in our handling of the compound, and make sure that an inert environment is maintained for storage whenever possible.

Another further direction of research could involve using SQUID equipment to study the magnetic properties of \compound. By investigating the effects of temperature and other external stimuli on the compound using SQUID measurements, we may be able to gain a deeper understanding of its spin crossover behavior and the underlying mechanisms at play.  Specifically, we could try and run SQUID on our sample to determine the transition temperature and find out where the spin-crossover temperature is.


\subsubsection{Computational}
One possible direction for further research on \compound would be to use more advanced computational methods, such as crystal-corrected density functional theory (DFT) calculations, to better understand the mechanisms underlying the spin crossover process. These calculations can provide detailed insights into the electronic structure of \compound and can help identify the key factors that influence the spin crossover process.

Specifically, we hope to use ORCA 5.0.2 to obtain vibrational DFT calculations for \compound to predict Raman and IR shift.  Eventually, we hope to incorporate the solid-state crystal properties of the material to obtain more accurate vibrational calculations.

In addition, it would be interesting to investigate the effects of temperature and other external stimuli on the electronic structure of \compound using computational methods. This could provide valuable information about how \compound responds to different environmental conditions, which could be useful for applications in which the spin crossover process is used as a switch or sensor. Further research in this area could also help identify potential strategies for optimizing the performance of \compound in such applications.
